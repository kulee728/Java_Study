...
public class UnitCavalry extends Unit{
	...
	public UnitCavalry(String name, int attackDamage, int attackRange, int movementSpeed,int hp) {
		super(name, attackDamage, attackRange,movementSpeed,hp);
	}

	@Override
	public int attack() {
		AttackGenerator atkG = new AttackGenerator(this.attackDamage);
		int resultDamage = atkG.getResultDamage();
		String attackMessage = atkG.getMessage();
		System.out.println("기병 "+name+"! 창을 들어 공격합니다!");
	...
	public void move() {
		...
		System.out.println("기병 "+name +","+" 이동을 시도합니다.");
